...
window.WorkspaceTailscale=(()=>{
 ...
 function render(state){
  ...
  const validLink=!connected&&/^https:\/\/login\.tailscale\.com\/a\/[A-Za-z0-9]+$/.test(state.loginUrl||'');
  $('[data-ts-message]').textContent=connected?'Tailscale 연결이 완료되었습니다.':state.error||(validLink?'아래 URL을 본인의 브라우저에서 열어 로그인하세요. 완료되면 연결 상태가 자동으로 갱신됩니다.':state.pending?'로그인 링크를 발급하는 중입니다…':'로그인 시작을 누르면 인증 URL이 표시됩니다.');
  const link=$('[data-ts-link]');link.hidden=true;link.removeAttribute('href');link.textContent='';
  if(validLink){link.href=state.loginUrl;link.textContent=state.loginUrl;link.hidden=false;}
 ...
 return {
  init(helpers){ui=helpers;dialog=document.createElement('dialog');dialog.className='editor-dialog';dialog.setAttribute('aria-label','Tailscale 설정');
   dialog.innerHTML='<header class="panel-head"><h2>Tailscale 설정</h2><button data-ts-close aria-label="닫기">×</button></header><div style="padding:16px"><h3 data-ts-status role="status">연결 확인 중…</h3><p data-ts-host></p><p class="section-hint">표시된 URL을 클릭하거나 복사해 본인의 PC 또는 휴대폰 브라우저에서 로그인하세요. 서버에서 브라우저를 열거나 대신 로그인하지 않습니다.</p><p data-ts-message role="status"></p><p><a data-ts-link target="_blank" rel="noopener noreferrer" style="overflow-wrap:anywhere" hidden></a></p><div class="actions"><button class="primary" data-ts-login>로그인 시작 / 연결</button><button data-ts-refresh>새로고침</button><button class="danger" data-ts-logout>로그아웃</button></div><p class="section-hint">로그아웃하면 Tailscale을 사용하는 장비 연결이 끊어질 수 있습니다. 대시보드 로그인에는 영향을 주지 않습니다.</p></div>';
