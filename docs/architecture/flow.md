...
장비 네트워크 선택과 점프 장비 순서 지정 → 저장/SSH 등록 → TAILSCALE이면 공유 tailscale0 인터페이스 주소 확인 → 첫 점프 장비부터 SSH 호스트 키 검증·인증 → 각 점프 장비에서 Direct-TCPIP 채널로 다음 주소에 연결 → 대상 장비 호스트 키 검증·인증 → SFTP/터미널/원격 어댑터 연결. 점프 체인은 최대 5개이며 어느 홉이라도 실패하면 전체 연결을 실패시킨다. 실패 시 기본 네트워크로 재시도하지 않는다. 프로필 변경 전 열린 실행 세션은 생성 당시 설정을 유지하므로 새 연결로 적용한다.
