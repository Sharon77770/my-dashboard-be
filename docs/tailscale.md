...
대상 장비에도 Tailscale 연결과 접근 정책 허용이 필요하다. 일반 SSH 서버·계정·비밀번호와 저장된 호스트 키 검증을 계속 사용하며, Tailscale SSH 전용 인증/승인 정책을 새로 구성하지 않는다. Tailscale 모드에서 Wake-on-LAN 브로드캐스트는 전송하지 않는다.
